...
if (import.meta.vitest) {
  ...
  describe("useSetting", () => {
    ...
    it("should have valid scoreboard structure", () => {
      ...
      expect(defaultValue.scoreboard.design.color).toBe(
        ScoreboardColorsMap["dual"][0]
      )

      // Camera and Logo
      expect(defaultValue.scoreboard.cameraAndLogo).toHaveProperty(
      ...
      expect(
        defaultValue.scoreboard.cameraAndLogo.displayCameraAndTwitterID
      ).toBe(false)
      ...
      expect(
        Array.isArray(defaultValue.scoreboard.cameraAndLogo.logoURLs)
      ).toBe(true)
